...
function MageNuggetsClassicFireblast_OnUpdate(self, elapsed) -- ignite
 self.TimeSinceLastUpdate = self.TimeSinceLastUpdate + elapsed;
    if (self.TimeSinceLastUpdate > 0.1) then
      local currentFbCharges, _, cooldownStart, cooldownDuration = GetSpellCooldown(108853);
      local fbCooldown = RoundOne(cooldownStart + cooldownDuration - GetTime());
      local currentPfCharges, _, cooldownStart, cooldownDuration = GetSpellCooldown(194466);
      ...
      if(currentFbCharges == 2) then
        ...
        if(currentFbCharges == 2) and (MageNuggetsClassic.FireNuggetNotifyBorderToggle == true)then
...
--
function MageNuggetsClassicHS_OnUpdate(self, elapsed)
 self.TimeSinceLastUpdate = self.TimeSinceLastUpdate + elapsed;
    if (self.TimeSinceLastUpdate > 0.1) then
...
function MageNuggetsClassicPyroclasm_OnUpdate(self, elapsed)
 self.TimeSinceLastUpdate = self.TimeSinceLastUpdate + elapsed;
    if (self.TimeSinceLastUpdate > 0.1) then
...
function MageNuggetsClassicHU_OnUpdate(self, elapsed)
 self.TimeSinceLastUpdate = self.TimeSinceLastUpdate + elapsed;
    if (self.TimeSinceLastUpdate > 0.1) then
